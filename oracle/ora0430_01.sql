...
-- 컬럼8개 - sno,name,kor,eng,math,total,avg,rank
select * from stuscore;
select count(*) from stuscore;

select * from stuscore where avg>=80;

-- 평균80점 이상이면서, 국어점수가 90점이상을 검색하시오.
...
select employee_id,hire_date from employees where employee_id>=150 or hire_date<'07/01/01';
...
-- member테이블에서, 날짜 25년 1월~3월까지 가입한 회원을 출력하시오.

select mdate from member order by mdate asc;
select mdate from member 
where mdate >='25/01/01' and mdate <= '25/03/31' order by mdate asc;

select mdate from member 
where mdate  between '25/01/01' and '25/03/31';


-- 이름중에 3번째 g가 들어간 사람을 검색하시오.
select * from stuscore;
...
-- salary, 년봉, 1438원 ,한화 천단위 표시 -> 대소문자 구분없이  3번째 g 들어가 있는 사원만 출력하시오.
